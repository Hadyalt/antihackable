        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                try:
                    ...
                    logger.log_entry("system", "Database Error", f"Login DB failure: {e}", "Yes")
                    print(sanitize_output(" Database connection error. Try again later."))
                    return None, None
                ...
                if not found_enc_username:
                    ...
                    try:
                        ...
                        logger.log_entry(username, "Database Error", f"Login query failed: {e}", "Yes")
                        print(sanitize_output(" A database error occurred."))
                        conn.close()
                        continue
                    ...
                    if result:
                        try:
                            ...
                            logger.log_entry(username, "Decryption Error", f"Error decrypting user data: {e}", "Yes")
                            conn.close()
                            continue
                        ...
                        if verify_password(password, stored_hash):
                            ...
                            if login.user_attempts[username] >= 5:
                                ...
                                try:
                                    ...
                                    logger.log_entry(username, "Database Error", f"Failed to deactivate account: {e}", "Yes")
                                conn.close()
                ...
                logger.log_entry("system", "Runtime Error", f"{type(e).__name__}: {e}", "Yes")
                print(sanitize_output(f" An unexpected error occurred: {e}"))
                continue
        return None, None
...
def show_main_menu(role, username):
    ...
    try:
        if role in ["superadmin", "systemadmin"]:
            ...
            if (
                ...
                try:
                    ...
                    logger.log_entry(username, "Log Error", f"Failed reading log: {e}", "Yes")
        ...
        print(sanitize_output(f" Error loading alerts: {e}"))

    while True:
        try:
            print("\n" + sanitize_output("=" * 50))
            print(
                sanitize_output(f"🛴 URBAN MOBILITY SYSTEM - Logged in as: {role.upper()}")
            )
            ...
            EncryptedLogger().log_entry(username, "Menu Error", f"{type(e).__name__}: {e}", "Yes")
            print(sanitize_output(f"An unexpected error occurred: {e}"))
...
def pre_login_menu():
    while True:
        try:
            ...
            EncryptedLogger().log_entry("system", "Menu Error", f"{type(e).__name__}: {e}", "Yes")
            print(sanitize_output(f"Unexpected error: {e}"))
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            EncryptedLogger().log_entry("system", "Fatal Error", f"{type(e).__name__}: {e}", "Yes")
            print(sanitize_output(f"Fatal error: {e}"))
